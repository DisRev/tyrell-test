...
FROM
    ...
    INNER JOIN personalities Personalities ON (Personalities.id = (JobsPersonalities.personality_id)
        AND (Personalities.deleted) IS NULL)) ON Jobs.id = (JobsPersonalities.job_id)
        LEFT JOIN
    ...
    INNER JOIN practical_skills PracticalSkills ON (PracticalSkills.id = (JobsPracticalSkills.practical_skill_id)
        AND (PracticalSkills.deleted) IS NULL)) ON Jobs.id = (JobsPracticalSkills.job_id)
        LEFT JOIN
    ...
    INNER JOIN basic_abilities BasicAbilities ON (BasicAbilities.id = (JobsBasicAbilities.basic_ability_id)
        AND (BasicAbilities.deleted) IS NULL)) ON Jobs.id = (JobsBasicAbilities.job_id)
        LEFT JOIN
    (jobs_tools JobsTools
    INNER JOIN affiliates Tools ON (Tools.type = 1
        AND Tools.id = (JobsTools.affiliate_id)
        AND (Tools.deleted) IS NULL)) ON Jobs.id = (JobsTools.job_id)
        LEFT JOIN
    ...
    INNER JOIN affiliates CareerPaths ON (CareerPaths.type = 3
        ...
        AND (CareerPaths.deleted) IS NULL)) ON Jobs.id = (JobsCareerPaths.job_id)
        LEFT JOIN
    ...
    INNER JOIN affiliates RecQualifications ON (RecQualifications.type = 2
        ...
        AND (RecQualifications.deleted) IS NULL)) ON Jobs.id = (JobsRecQualifications.job_id)
        LEFT JOIN
    ...
    INNER JOIN affiliates ReqQualifications ON (ReqQualifications.type = 2
        ...
        AND (ReqQualifications.deleted) IS NULL)) ON Jobs.id = (JobsReqQualifications.job_id)
        INNER JOIN
